...
/**
 * This class mirrors {@link ImmutableAtomicTest} and tests the methods which operate on an
 * {@link AtomicReferenceFieldUpdater}.
 */
public class ImmutableAtomicUpdaterTest {
    ...
    private interface UpdateFunction<T> {
        boolean apply(Holder<T> obj, Function<T, T> transformer);
    }

    private static final class Holder<T> {
        private volatile T data;
        @SuppressWarnings("rawtypes")
        private static final AtomicReferenceFieldUpdater<Holder, Object> UPDATER =
            AtomicReferenceFieldUpdater.newUpdater(Holder.class, Object.class, "data");
        ...
        public AtomicReferenceFieldUpdater<Holder<T>, T> updater() {
            @SuppressWarnings(Suppressions.UNCHECKED_SAFE)
            AtomicReferenceFieldUpdater<Holder<T>, T> result = (AtomicReferenceFieldUpdater<Holder<T>, T>)(AtomicReferenceFieldUpdater)UPDATER;
            return result;
        }
    ...
    public void update_StartWithNull() {
        updateHelper(new Consumer<UpdateFunction<ImmutableTreeList<Integer>>>() {
            ...
            public void accept(UpdateFunction<ImmutableTreeList<Integer>> func) {
                Holder<ImmutableTreeList<Integer>> list = new Holder<ImmutableTreeList<Integer>>(null);
    ...
    public void update_IncrementalUpdate() {
        updateHelper(new Consumer<UpdateFunction<ImmutableTreeList<Integer>>>() {
            @Override
            public void accept(UpdateFunction<ImmutableTreeList<Integer>> func) {
                Holder<ImmutableTreeList<Integer>> list = new Holder<ImmutableTreeList<Integer>>(ImmutableTreeList.create(1));
                assertTrue(func.apply(
                    list,
                    new UnaryOperator<ImmutableTreeList<Integer>>() {
                        @Override
...
//        }
    ...
    public void getOrAddMapValue() {
        Holder<ImmutableHashMap<Integer, String>> map = new Holder<ImmutableHashMap<Integer, String>>(ImmutableHashMap.<Integer, String>create());
        String value = ImmutableAtomic.getOrAdd(map.updater(), map, 1, "a");
        assertEquals("a", value);
        value = ImmutableAtomic.getOrAdd(map.updater(), map, 1, "b");
        assertEquals("a", value);
    }
    ...
    public void tryAddMap() {
        ...
        assertTrue(ImmutableAtomic.tryAdd(map.updater(), map, 1, "a"));
        assertEquals("a", map.get().get(1));

        assertFalse(ImmutableAtomic.tryAdd(map.updater(), map, 1, "a"));
        assertFalse(ImmutableAtomic.tryAdd(map.updater(), map, 1, "b"));
        assertEquals("a", map.get().get(1));
    }
    ...
    public void tryRemoveMap() {
        ...
        assertNull(ImmutableAtomic.tryRemove(map.updater(), map, 1));

        map.set(map.get().add(1, "a"));
        assertEquals("a", ImmutableAtomic.tryRemove(map.updater(), map, 1));
        assertTrue(map.get().isEmpty());
    }

    @Test
    public void pushStack() {
        Holder<ImmutableLinkedStack<Integer>> stack = new Holder<ImmutableLinkedStack<Integer>>(ImmutableLinkedStack.<Integer>create());
        ImmutableAtomic.push(stack.updater(), stack, 5);
        assertFalse(stack.get().isEmpty());
        assertEquals(5, (int)stack.get().peek());
        assertTrue(stack.get().pop().isEmpty());

        ImmutableAtomic.push(stack.updater(), stack, 8);
    ...
    public void tryPopStack() {
        ...
        assertNull(ImmutableAtomic.tryPop(stack.updater(), stack));
        stack.set(stack.get().push(2).push(3));
        assertEquals(3, (int)ImmutableAtomic.tryPop(stack.updater(), stack));
    ...
    public void tryPollQueue() {
        Holder<ImmutableLinkedQueue<Integer>> queue = new Holder<ImmutableLinkedQueue<Integer>>(ImmutableLinkedQueue.<Integer>create());
        assertNull(ImmutableAtomic.tryPoll(queue.updater(), queue));

        queue.set(queue.get().add(1).add(2));
        assertEquals(1, (int)ImmutableAtomic.tryPoll(queue.updater(), queue));
        assertEquals(2, (int)ImmutableAtomic.tryPoll(queue.updater(), queue));
        assertNull(ImmutableAtomic.tryPoll(queue.updater(), queue));
    }
    ...
    public void addQueue() {
        Holder<ImmutableLinkedQueue<Integer>> queue = new Holder<ImmutableLinkedQueue<Integer>>(ImmutableLinkedQueue.<Integer>create());
        ImmutableAtomic.add(queue.updater(), queue, 1);
        assertEquals(1, (int)queue.get().peek());
        assertTrue(queue.get().poll().isEmpty());

        ImmutableAtomic.add(queue.updater(), queue, 2);
        assertEquals(1, (int)queue.get().peek());
    ...
    private static <T> void updateHelper(Consumer<UpdateFunction<T>> test) {
        test.accept(new UpdateFunction<T>() {
            @Override
            public boolean apply(Holder<T> location, Function<T, T> transformer) {
                return ImmutableAtomic.update(location.updater(), location, transformer);
            }
        });
        test.accept(new UpdateFunction<T>() {
            @Override
            public boolean apply(Holder<T> location, Function<T, T> transformer) {
                return updateWrapper(location.updater(), location, transformer);
            }
        });
    }
     ...
     */
    private static <C, T> boolean updateWrapper(AtomicReferenceFieldUpdater<C, T> updater, C obj, final Function<T, T> transformer) {
